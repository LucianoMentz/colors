...
def rgb_to_hex(rgb_color):
    ...
    return "#"+"".join(["0{0:x}".format(v) if v < 16 else
                        "{0:x}".format(v) for v in rgb_color])
...
def color_dict(gradient):
    """ Takes in a list of RGB sub-lists and returns dictionary of
      ...
      defined later on """
    return {"hex": [rgb_to_hex(rgb_color) for rgb_color in gradient],
            "r": [rgb_color[0] for rgb_color in gradient],
            "g": [rgb_color[1] for rgb_color in gradient],
            "b": [rgb_color[2] for rgb_color in gradient]
            }
...
def polylinear_gradient(colors, n):
    ...
    if len(colors) > 1:
        for col in range(1, len(colors) - 1):
            next_color = linear_gradient(colors[col], colors[col+1], n_out)
            for k in ("hex", "r", "g", "b"):
                # Exclude first point to avoid duplicates
                gradient_dict[k] += next_color[k][1:]
